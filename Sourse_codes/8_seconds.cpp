...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, int) {
    ...
    int ret1 = MessageBox(nullptr,
        "8 seconds.exe: This runs GDI for 8 seconds.\nPress Ok to execute.",
        "8 seconds.exe",
        MB_OKCANCEL | MB_ICONWARNING);
    if (ret1 == IDCANCEL) return 0;

    Sleep(500); // This is the timer before it makes Audio and GDI sync

    // ===== AUDIO SETUP =====
    const int sampleRate = 8000;
    const int seconds = 8; // ~15 seconds
    ...
    while (GetTickCount() - start < seconds * 1000) {
        ...
        if (GetTickCount() - lastSwitch > 10000) {
            effect = (effect + 1) % 5; // switch effect every 10 sec
